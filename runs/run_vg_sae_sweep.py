"""Train the Experiment 07 SAE sweep as independent, parallel jobs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seeds", help="Optional comma-separated seed override.")
    ...
    logging.add_argument("--no-wandb", action="store_const", const="disabled", dest="wandb_mode")
...
def _preflight_wandb(mode: str) -> None:
    if mode == "disabled" or mode == "offline":
        return
    import wandb
...
def train_one(run_dir: Path, device: str, wandb_mode: str, force: bool) -> None:
    ...
    try:
        ...
        metadata = {
            ...
            "train_device": device,
            "train_source_fingerprint": bundle["train_provenance"][
                "source_fingerprint"
            ],
        }
        write_rows(
...
def prepare_runs(
    ...
    if manifest_path.exists():
        ...
        if fingerprint(previous["sweep_config"]) != fingerprint(config_dict):
    ...
    for spec in specs:
        ...
        if config_path.exists() and read_json(config_path).get("fingerprint") != bundle["fingerprint"]:
            if not force:
                raise ValueError(f"Configuration collision in {run_dir}; use --force or a new output.")
        write_json(config_path, bundle)
        relative_dir = str(run_dir.relative_to(sweep_dir))
        entries[spec.run_id] = {"run_id": spec.run_id, "relative_dir": relative_dir}
...
def main(args: argparse.Namespace) -> int:
    if args.worker:
        ...
        try:
            ...
            run_fingerprint = read_json(config_path).get("fingerprint") if config_path.exists() else None
            write_json(
                args.run_dir / "train_status.json",
                {"state": "failed", "fingerprint": run_fingerprint, "failed_at": utc_now(), "error": repr(error)},
            )
            raise
        return 0
    ...
    default_name = (
        ...
        if args.config is not None
        else f"exp07_rho_model_comparison{suffix}"
    )
    ...
    incomplete = [
        ...
        if not _is_complete(run_dir, read_json(run_dir / "config.json")["fingerprint"])
    ]
