...
각각 `(run_id, stage, world, rho, p, stream_role)`에서 독립 생성한다. 동일 cell의
...
alignment, selection IDs와 calibration-only amplitude gains를 hash로 고정한다.
...
새 selection 후보는 24k/28k/32k 세 개로 교체한다. 32k에서도 부족하면 unconverged로
표시하고 그 comparator에 대한 확정 우위를 보류한다. 추가 LR×warmup factorial은 없다.

### 새 held-out 직접 비교와 density transfer

B2 development를 마친 뒤 방법을 고정한다. 방법별 최종 3 controls는 개발 관측에서
target 2.0을 먼저 포함하고 1.8/2.2까지 coverage를 최대화하도록 골라 수치 manifest에
기록한다. VG는 B2의 same-gamma risk control 2를 반드시 포함하며 남은 두 슬롯을
같은 규칙으로 선택한다. 따라서 세 target를 동시에 못 채울 수도 있으며 이를 숨기지
...
**판정:** useful region은 signed cosine≥.95, support F1≥.95인 점과 EV를 함께
...
checkpoint 규칙을 그대로 적용한다. 개발의 `S_source` score로 precision 후보를
선택하되 signed cosine≥.95를 유지하면서 profiled 대비 cosine 손실≤.01,
...
주 대조는 gamma=2에서 fixed/global 대비 profiled의 same-gamma total effect다.
...
| 순서 | 작업 | 새 training fits 상한 | Selection snapshots 상한 | 진행 조건 |
|---|---|---:|---:|---|
| M0 | 5 methods × 500-step throughput probe, metric/readout/normalization 검증 | 5 probes | scientific 후보 0 | config hash, 시간/메모리, baseline native inference 확인 |
...
M1–M5는 최대 **248 scientific fits**, 최대 **744 selection snapshots**다.
그중 development 62 fits/186 snapshots에는 test가 없고, main/transfer 186 fits의
558 snapshots를 fresh test에서 동결된 방식으로 평가한다. Throughput probes는 별도다.
