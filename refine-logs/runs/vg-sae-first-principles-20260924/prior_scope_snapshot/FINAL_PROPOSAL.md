...
2026-09-22. 세 파일럿의 근거를 바탕으로 한 현재 방법 제안과 다음 개발 대조다.
...
Rescale 후에도 VG coefficient NMSE가 더 낮은 pair는4/5다. Negative world211에서는
VG .0001063 대 rescaled L1 .0132759지만, independent world212는 rescaled L1
.00009558이 VG .00012410보다 낮다. 이 제한된 amplitude-fidelity 신호를 보존하되
dictionary/support joint 우위나 같은 capacity의 고유 objective 효과로 확대하지 않는다.
...
따라 유한 하한이 없다. 실제 **profiled branch**의 epsilon은 이 발산을 제한한다. Global learned-beta branch는
energy를 epsilon으로 clamp하지 않으므로 그 branch에도 수치 floor가 있다고 쓰지 않는다.
유한 부동소수점의 overflow와 objective의 수학적 하한은 구분한다. 숫자와 production-epsilon tests를 보존했다.

Fixed finite beta에서는 E≥0와 KL≥0로 Gaussian 상수의 유한 하한이 있다.
이는 해당 무한 loss 감소를 제거하는 수식 성질이며 feature/support recovery의 보장이 아니다.
이 family가 실제 SGD의 원인이었다고 확인한 것도 아니다. 단지 같은 구조에서
precision policy를 분리할 구체적인 근거이며 독창적인 정리로 내세우지 않는다.
자세한 가정과 수치는 `idea-stage/runs/vg-sae-sparse-but-wrong-20260922/evidence/dense_offset_witness.md/json`에 있다.
...
상세 실행은 [EXPERIMENT_PLAN](EXPERIMENT_PLAN.md), 현재 완료 상태는
[EXPERIMENT_TRACKER](EXPERIMENT_TRACKER.md), 참조 논문은
[REF_PAPER_SUMMARY](../idea-stage/REF_PAPER_SUMMARY.md)에 있다.
