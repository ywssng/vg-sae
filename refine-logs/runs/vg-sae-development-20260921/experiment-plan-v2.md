...
| C2 | 현재 VG parameterization의 minibatch-profiled beta recipe에서 variance와 entropy의 support–amplitude 결합 및 hard-code 기여를 각각 판정한다. | 같은 beta mode·architecture·탐색 기회의 2×2 deletion, m/a scale과 mean/stochastic/hard 지표 | B2 |
...
- L1 primary는 **native ReLU code**, support는 `h > 0`이다. P1의 train-GMM
  ...
  GMM은 고유하게 선택된 L1 checkpoint마다 해당 checkpoint의 train activations로
  한 번 fit하고 cal/test에서 refit하지 않는다.
...
C2는 **profiled-beta recipe에 한정**하며 아래 두 비교를 별도로 판정한다.
B1에서 learned-beta recipe가 선택돼도 이 C2 결과를 그 recipe에 자동으로 옮기지 않는다.
두 비교는 `full vs no-variance`와 `full vs no-entropy`다. 각 비교의 지지 기준은 같다.

1. 세 world 모두 full과 해당 deletion에 cal cap 8 feasible 후보가 있고 test mean L0도
   cap 8 이내여야 한다. Test를 본 재선택은 하지 않는다.
2. 세 world 중 2개 이상에서 full의 hard `E_z`가 낮고, paired 상대 감소 평균이
   3% 이상이며 F1의 평균 하락은 0.02 이내여야 한다.
3. m/a scale과 mean/stochastic/hard risk가 그 항의 기여라는 해석과 일치해야 한다.
   수치 기준만으로 모순된 진단을 무시하지 않는다.

| Variance 비교 | Entropy 비교 | 허용되는 C2 판정 |
| --- | --- | --- |
| 지지 | 비지지/유보 | `variance-supported` — variance에 관한 결론만 |
| 비지지/유보 | 지지 | `entropy-supported` — entropy에 관한 결론만 |
| 지지 | 지지 | `both-supported` — 두 항 각각의 기여 |
| 비지지/유보 | 비지지/유보 | `neither-supported` — 어느 항도 이 기준으로 지지하지 못함 |

각 비지지와 유보 이유는 따로 기록한다. Feasible point 부재·collapse·NaN 때문에
비교 불가능한 경우를 유효한 비교의 영가설 결과와 합치지 않는다. Collapse만 관측되면
수치적 superiority gate의 통과로 세지 않고, **이 parameterization에서 해당 항을
제거할 때 coupling이 손상될 수 있다는 제한된 관찰**로 기술한다. 모든 paired 차이와
both-deleted arm, 2×2 interaction은 기술적으로 공개하며 interaction의 별도 기여 주장을
하려면 추가 사전 판정 규칙이 필요하다. Deletion failure는 일반 deterministic baseline에
대한 승리나 calibrated uncertainty의 증거가 아니다. 비유한 수치, 0 firing,
급격한 amplitude 증가도 결과이며 test 확인 후 clamp/penalty를 추가하지 않는다.
...
metric은 learned latent마다 best-absolute-cosine true feature와 비교하며 중복 matching을
허용한다. Coverage는 이 geometric matching의 unique true feature 비율이다.
4,096/16,384의 불완전 coverage 영향을 별도로 설명한다.
