...
- P3b:같은 finite-correction 선택지의 병렬 계산 후속을 별도 fresh split에서 확인 중이다.
  이것은 P3를 본 뒤 추가한 탐색이며, 기존 P3의 사전등록 결과를 대체하지 않는다.

수치·설정·원본은 `idea-stage/runs/vg-sae-development-20260921/evidence/pilots/`.
...
- **C2:** 그 이점 중 일부는 단순한 두 encoder 구조·추가 parameter·더 많은 tuning이
  아닌 full VG support-risk objective의 기여다. variance×entropy를 동일 architecture에서
  각각 제거하고, 각 variant를 독립 calibration해 검사한다. 현재 미검증이다.
...
학습은 먼저 각 method의 한 run에서 고정 budget extension과 final plateau를 확인한다.
VG만 더 오래 돌려 이기게 하지 않고 official baseline의 auxiliary loss·schedule을 보존한다.
큰 비교 전에 cal로 training recipe·control grid·budget을 고정한다.
평균5% 이상 error 개선과 F1 하락 .02 이내를 다음 단계의 operational go 기준으로 둔다.
세 seed 값과 범위를 모두 보고하고, 작은 표본의 screening을 통계적 확증이라고 부르지 않는다.

### B2: 원래 objective의 필요성

같은 VG encoder/decoder에서 full, no-variance, no-entropy, 둘다제거의4variants.
gamma는 각각 같은 HPO budget으로 cal에서 고르고 실제L0와 beta를 기록한다.
손실값은 서로 다른 objective이므로 직접 우열을 판정하지 않는다.
native hard coefficient error/F1와 mean/sample/hard reconstruction을 사용한다.
같은 architecture/parameter count의 deletion check라 C2를 직접 겨냥한다.
divergence·collapse는 숨기지 않으며 test에 맞춰 새 clamp나 penalty를 추가하지 않는다.
...
평가하는 것이 먼저다. fresh cal/test text, native hard L0, x fidelity, CE/KL를 본다.
real support 정답이 없으므로 semantic calibration·true recovery를 주장하지 않는다.
이 단계에서 이득이 없으면 적용 범위를 제한하며 주 연구를 다른 진단 논문으로 바꾸지 않는다.

## 다음 실행과 예산

현재 run의 새 pilot budget은8GPUh 이내이며, P1/P2/P3 기록 합계는 약.288GPUh다
(import/개별 smoke 제외, P3b는 종료 후 추가). 대규모 새학습은 이번 run에서 하지 않았다.
