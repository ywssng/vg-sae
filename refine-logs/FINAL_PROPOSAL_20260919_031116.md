...
3. 실제 개입에서도 loss 선호가 확인된 경우에만, 같은 폭·계산량의 대조군과
   ...
   복제 선호가 없거나 손상이 없으면 해당 조건의 비지지 또는 판단 유보로 기록한다.
...
source는10000-step checkpoint를 기본으로 선택한다. source training의 checkpoints 6000/8000/10000에서 cal R,V,K,beta와 signed margin을
...
cal pair가 없으면 해당 checkpoint와 소속 data world를 inconclusive로 둔다.
3개 optimizer seed 중 실패한 것을 world 평균에서 조용히 제외하지 않는다. 높은 test 이득으로 바꾸지 않는다.
...
frequency 구간·포함률을 보고한다. 고정 eligible 집합 중 test positive가0인 feature 비율이5%를 넘으면
해당 B3 semantic 비교는 inconclusive다. 5% 이하는 정의 가능한 AP의 평균과
동일한 고정 집합 대비 누락률을 함께 보고하며 arm별로 분모를 바꾸지 않는다.
